...
_Bool add_record_table(const char *table_name, int user_id, const char *record_values) {
    ...
    _Bool is_valid = validate_record_values(existingTable, record_values);
    if (!is_valid) {
        free(existingTable);
        return false;
    }

    _Bool success = data_table_add_record(table_name, user_id, record_values);
    free(existingTable);
    return success;
}
...
_Bool delete_record_table(const char *table_name, int user_id, int record_id) {
    ...
    _Bool success = data_table_delete_record(table_name, user_id, record_id);
    free(existingTable);
    return success;
}
